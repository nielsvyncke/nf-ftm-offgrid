"""
This script processes the GlaS dataset to generate a datalist JSON file in a Decathlon-style format.
...
def make_datalist():
    ...
    label_list = sorted(glob.glob("*_anno.bmp", recursive=True))

    # Make training set
    datalist = []
    for label in label_list:
        id_ = os.path.basename(label).replace("_anno.bmp", "")
        img = os.path.basename(label).replace("_anno", "")
        sample = {"id": id_, "image": img, "label": label}
...
def roi_area(mask):
    ...
    roi_num_pixels = torch.sum(mask > 0)
...
def main(args):
    ...
    # Create datalists for GlaS dataset
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Prepare datalist for GlaS dataset.")
    parser.add_argument(
