"""Generic, configurable U-Net encoder--decoder.

The backbone is block-agnostic: each stage's block type can be supplied by the
caller (e.g. the NF-FTM ``TransformerStage``), defaulting to a ``DoubleConv``
convolutional block. This module is self-contained and depends only on
``scripts.layers``.
"""
...
from .layers import as_tuple, partialize, DoubleConv


class Same(object):
    """Helper that returns the same block for every stage index."""
...
class UNetStage(nn.Module):
    """A single U-Net stage: ``depth`` stacked blocks."""
...
class UNetEncoderBlock(nn.Module):
    """Downsample followed by a stage of blocks."""
...
class UNetEncoder(nn.Module):
    """U-Net encoder: a cascade of downsampling stages."""
...
class UNetDecoderBlock(nn.Module):
    """Upsample, concatenate skip connection, then a stage of blocks."""
...
class UNetDecoder(nn.Module):
    """U-Net decoder: a cascade of upsampling stages with skip connections."""
...
class UNet(nn.Module):
    """Generic U-Net architecture with configurable per-stage blocks."""
